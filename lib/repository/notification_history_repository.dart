...
class NotificationHistoryRepository {
  ...
  Future<void> insert(NotificationRecord record) async {
    return _lock.synchronized(() async {
      try {
        ...
        developer.log(
          '[NotificationHistoryRepository] Inserted notification record for geofence: ${record.geofenceId}',
        );
      } catch (e, stackTrace) {
        developer.log(
  ...
  Future<List<NotificationRecord>> getAll() async {
    try {
      ...
      final records =
          maps.map((map) => NotificationRecord.fromMap(map)).toList();
      developer.log(
        '[NotificationHistoryRepository] Fetched ${records.length} notification records.',
      );
      return records;
    } catch (e, stackTrace) {
      developer.log(
  ...
  Future<void> clearAll() async {
    return _lock.synchronized(() async {
      try {
        ...
        developer.log(
          '[NotificationHistoryRepository] Cleared all notification records.',
        );
      } catch (e, stackTrace) {
        developer.log(
